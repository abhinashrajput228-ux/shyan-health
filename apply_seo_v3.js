...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  // Body changes - replace h1 manually
  const h1Start = html.indexOf('<h1>');
  const h1End = html.indexOf('</h1>') + 5;
  if (h1Start !== -1 && h1End !== -1) {
    html = html.substring(0, h1Start) + '<h1>Natural Ayurvedic Weight Loss for Indian Women | Fat to Fit Journey</h1>' + html.substring(h1End);
  }
  
  // Image alts - simpler replace
  html = html.split('alt="Ayurvedic Natural Journey Transformation"').join('alt="Indian woman\'s natural Ayurvedic weight loss journey – Shyan Health"');
  html = html.split('alt="Natural Ayurvedic Ingredients Shyan Health"').join('alt="Natural Ayurvedic herbs – ashwagandha, shatavari, triphala by Shyan Health"');
  html = html.split('alt="Fat to Fit Ayurvedic Weight Loss Tablets"').join('alt="Fat to Fit Tablets – Natural Ayurvedic Weight Loss Supplements for Women"');
